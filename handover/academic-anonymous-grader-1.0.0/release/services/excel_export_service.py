...
def _build_final_grades_sheet(
    ...
    for sub in submissions:
        ...
        mat_obj = assessment.material
        dept_label = mat_obj.ref_department.display_name if mat_obj and mat_obj.ref_department else ""
        stage_label = mat_obj.ref_stage.display_name if mat_obj and mat_obj.ref_stage else ""
        term_label = mat_obj.ref_term.display_name if mat_obj and mat_obj.ref_term else ""
        year_label = mat_obj.ref_academic_year.display_name if mat_obj and mat_obj.ref_academic_year else ""
        ws.cell(row=row_num, column=13, value=_safe_value(dept_label))
        ws.cell(row=row_num, column=14, value=_safe_value(stage_label))
        ws.cell(row=row_num, column=15, value=_safe_value(term_label))
        ws.cell(row=row_num, column=16, value=_safe_value(year_label))
        ws.cell(row=row_num, column=17, value=now_str)
...
def _build_export_summary_sheet(
    ...
    summary_data = [
        ...
        ("Department", mat_obj.ref_department.display_name if mat_obj and mat_obj.ref_department else ""),
        ("Stage", mat_obj.ref_stage.display_name if mat_obj and mat_obj.ref_stage else ""),
        ("Term", mat_obj.ref_term.display_name if mat_obj and mat_obj.ref_term else ""),
        ("Academic Year", mat_obj.ref_academic_year.display_name if mat_obj and mat_obj.ref_academic_year else ""),

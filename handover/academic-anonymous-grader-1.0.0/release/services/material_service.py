...
class MaterialResult:
    ...
    created_at: str
    updated_at: str | None
    # Phase 12.1 — Classification reference IDs and labels
...
def _material_to_result(m: Material, assessment_count: int = 0) -> MaterialResult:
    return MaterialResult(
        ...
        academic_stage_id=m.academic_stage_id,
        academic_term_id=m.academic_term_id,
        academic_year_id=m.academic_year_id,
        department_label=m.ref_department.display_name if m.ref_department else None,
        stage_label=m.ref_stage.display_name if m.ref_stage else None,
        term_label=m.ref_term.display_name if m.ref_term else None,
        academic_year_label=m.ref_academic_year.display_name if m.ref_academic_year else None,
        classification_needs_review=m.classification_needs_review,
    )
...
def create_material(
    ...
    # Determine classification review status
    refs = [department_id, academic_stage_id, academic_term_id, academic_year_id]
    all_classified = all(r is not None for r in refs)
    ...
    material = Material(
        ...
        academic_stage_id=academic_stage_id,
        academic_term_id=academic_term_id,
        academic_year_id=academic_year_id,
        classification_needs_review=not all_classified,
    )
...
def update_material(
    ...
    # Update classification foreign keys
    if department_id is not None:
        m.department_id = department_id if department_id else None
    if academic_stage_id is not None:
        m.academic_stage_id = academic_stage_id if academic_stage_id else None
    if academic_term_id is not None:
        m.academic_term_id = academic_term_id if academic_term_id else None
    if academic_year_id is not None:
        m.academic_year_id = academic_year_id if academic_year_id else None

    # Auto-set classification_needs_review — clear only when all 4 refs valid
    all_refs = [m.department_id, m.academic_stage_id, m.academic_term_id, m.academic_year_id]
    m.classification_needs_review = not all(r is not None for r in all_refs)

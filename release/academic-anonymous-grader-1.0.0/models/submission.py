...
# Duration (minutes) after which a grading claim expires
GRADING_CLAIM_DURATION_MINUTES: int = 120
...
class Submission(TimestampMixin, Base):
    ...
    # Phase 10 — Grading claim / concurrency control
    assigned_grader_user_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True
    )
    grading_claimed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    grading_lock_expires_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )

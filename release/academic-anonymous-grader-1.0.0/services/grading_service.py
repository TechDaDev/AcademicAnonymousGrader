...
def save_submission_grades(
    ...
    *,
    ...
    When auth_ctx is provided for a grader role, enforces assignment
    access to prevent unassigned instructors from grading.
    """
    # Phase 10 — Enforce instructor assignment access
    if auth_ctx is not None and auth_ctx.role != "administrator":
        from services.assignment_service import require_assignment_access
        try:
            require_assignment_access(session, assessment_id, auth_ctx=auth_ctx)

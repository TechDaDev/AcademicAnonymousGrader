...
def main() -> None:
    ...
    # Step 1 — Select material (filtered by instructor assignment if grader)
    ...
    with session_scope(factory) as session:
        if role == "administrator":
            ...
            # Grader: only see materials from assigned assessments
            assigned = get_own_assigned_assessments(session, auth_ctx=auth_ctx)
            assigned_ids = {a.material_id for a in assigned if a.material_id}
            all_materials = list_materials(session, include_archived=False)
            materials = [m for m in all_materials if m.id in assigned_ids]

    mat_options = {m.name: m.id for m in materials} if materials else {}
    if not mat_options:
        if role == "grader":
            st.info("You have no assigned assessments. Contact an administrator.")
        else:
    ...
    # Step 2 — Select assessment (filtered by instructor assignment if grader)
    with session_scope(factory) as session:
        if role == "administrator":
            ...
            assigned = get_own_assigned_assessments(session, auth_ctx=auth_ctx)
            assessments = [a for a in assigned if a.material_id == material_id]
    ...
    # Step 2b — Enforce assignment access for graders
    if role == "grader":
        with session_scope(factory) as session:
            try:
                require_assignment_access(session, assessment.id, auth_ctx=auth_ctx)
            except Exception:
                render_safe_error("The requested grading work is unavailable.")
                st.stop()
